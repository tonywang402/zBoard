...
|---|---|---|
| **CI/CD Build Status** | Real-time color-coded pipeline cards for CircleCI and GitHub Actions. Cards are sorted by level (`high` → `medium` → `low`) first, then by urgency within each level. Hovering over a red GitHub Actions card shows a popover with the failed job names and their failed steps. | [BuildStatusMonitoring.instructions.md](instructions/BuildStatusMonitoring.instructions.md) |
