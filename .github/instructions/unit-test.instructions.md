...
- **Config-gated branching** (`enabled` flags on `circleCI`, `github`, `localData`, `apiTable`, `googleSheet`, `kanbanize`, `zendesk`, `datadog`) — each branch has a completely different data path or fallback to fake data.
- **Data transformation functions** (`sortMembers`, `calculateStartEndDate`, `buildCardInfo`, `getMonitorColor`) — pure functions containing non-trivial conditionals, optional chaining, and `||` fallback chains.
- **HTTP error handling** in `fetchPipelines`, `fetchWorkflows`, `fetchTickets`, and `getStatus` — each checks `!response.ok` and throws a formatted error.
